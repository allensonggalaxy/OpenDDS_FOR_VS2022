...
sub usageAndExit {
  ...
  print "Usage: $base [ChangeLog File] [user name] [email address]\n" .
        ...
        "        " . (' ' x length($base)) . "[-m <start_revision>]\n\n" .
        "       Uses git to determine which files are modified or added\n" .
        ...
        "       ChangeLog entry.\n" .
        "\n" .
        "       If -m is used, ChangeLog entries will be created based on\n" .
        "       the git log of revisions between <start_revision> and HEAD.\n";
  exit(0);
}
...
sub edit {
  my($status, $error, $unknown) = $editor->edit($file, @dirs, $revision);
...
my $status = (-f $file) ? edit() : 0;

if (! -f $file && -f "$modules[0]/$file") {
  my $start_dir = getcwd;
  for my $dir (@modules) {
